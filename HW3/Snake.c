...
void go(snake_t *head) {
    ...
    switch(head->direction){
        case LEFT:
            //было head->x = (head->x <= 0) ? max_x - 1 : head->x - 1;
            //стало:
            ...
            break;
        case RIGHT:
            //было head->x = (head->x >= max_x - 1) ? 0 : head->x + 1;
            //стало:
            if (head->x >= max_x - 1)
            {
				 head->direction = UP;
            ...
            break;
        case UP:
            //было head->y = (head->y <= MIN_Y) ? max_y - 1 : head->y - 1;
            //стало:
            ...
            break;
        case DOWN:
            //было head->y = (head->y >= max_y - 1) ? MIN_Y : head->y + 1;
            //стало:
...
void playGame() {
    ...
    //было timeout(100);
    //стало:
    //clock_t m1Clock = clock();
    //clock_t m1Clock1;
    //while(1)
    //{
		//clock_t m1Clock1 = clock();
		//if ((double)(m1Clock1-m1Clock)/(CLOCKS_PER_SEC/1000) > 100) break;
	//}
	//printf("$%lu == %lud\n", m1Clock1, m1Clock);
